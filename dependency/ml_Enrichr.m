...
if nargin < 3
    genesets = ["GO_Biological_Process_2025", ...
                ...
                "KEGG_2021_Human", ...%"CellMarker_2024",
...
%% ClustOra fix 2026
%Enrichr API sometimes returns strings containing hex escape sequences (like \xCE\xA3), 
% which are not standard in the JSON specification. While these are valid in some languages like JavaScript or Python, 
% MATLAB's jsondecode is strict and only supports standard escape sequences such as \uXXXX for Unicode
% Convert invalid \xHH escape sequences to valid \u00HH sequences
response = regexprep(response, '\\x([0-9a-fA-F]{2})', '\\u00$1');
%%
...
output=[output,cellstr(genesets')];
end
...
function [T] = in_response2T(response, gene_set_library, minumgene, pvaluecut)
        ...
        for k = 1:length(res)
            t = cell2table(res{k}', 'VariableNames', headertxt);assignin('base',"t",t)
            if numel(t.OverlappingGenes)>1% Feb 2026 added by John for VIZGEN
            s = sprintf("%s,", t.OverlappingGenes{1}{:}); 
            t.OverlappingGenes{1} = char(extractBefore(s, strlength(s)));
            else %this else allows if only one gene
            t.OverlappingGenes{1} = t.OverlappingGenes{1};
            end  %%%% Feb 2026
