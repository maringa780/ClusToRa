...
-`Colormap` <sub>**dropdown** menu used to define the colormap for plotting. Default: custom one called `ClusToRa_colormap`. An adjacent **dropdown** menu defines whether the color scale spans all available z-scores within a group (`Auto`), whole dataset (`Global`) or only the maximum score within the plotted interactions (`Local`).</sub> \
