...
-`Colormap` <sub>**dropdown** menu used to define the colormap for plotting. Default: custom one called `ClusToRa_colormap`. An adjacent **dropdown** menu defines whether the color scale spans all available z-scores within a group (`Auto`), maximum in whole dataset (`Global`), maximum score within the interactions to plot(`Local`), or `User` defined numeric value.</sub> \
...
---`file type` <sub>**dropdown** menu defining export format (`.pdf`, `.svg`, `.png`, `.tif`, `.jpg`, `.emf`, `.eps`). **Note:** `.pdf`, `.svg`,`.emf` and `.eps` format are exported as `vector graphics`  </sub> \
